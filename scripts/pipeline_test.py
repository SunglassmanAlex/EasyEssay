...
def main() -> None:
    ...
    try:
        ...
        check("形状正确时 span 被保留",
              ...
              .get("span") == 2)
        # 分页标记行：**2026-09-16 这条要求反转了**。
        # 早先用户说"去掉换页提示"，所以当时断言"导出里没有任何换页标记"；
        # 后来用户以参照成品稿为标准，而参照稿**有** `.row.pbreak`（13 条），
        # 导出页自己的说明文字也一直宣称"每两页之间有一条虚线分页标记行"。
        # 现在的要求：要有，且每条都**左右两栏都占位**（否则中英分界线会断）。
        # 校验靠真实浏览器（scripts/browser_check.py）—— 静态 HTML 里没有行，
        # 内容是 JS 现渲染的，只能在 DOM 上数。
