...
（Windows 是 `EasyEssay.exe`）——浏览器会自动打开，第一次让你填自己的 DeepSeek API Key，
填一次就记住了。**不需要注册、不需要登录。**
...
公式就少了括号，模型拿到残缺输入自然翻错。

现在改为**显式占位**：认不出的字形统一渲染成 `⟦?⟧`，并明确告诉模型
「这是大型括号/矩阵分隔符/分式线中最可能的那一种，必须结合上下文补齐，禁止原样保留」。
阅读页还会出现 **「修复乱码段（N）」** 按钮，只重译含占位符的那几段，不用全文重跑。
...
| 4. 字符→LaTeX | 内置 300+ 映射（希腊字母、关系符、大运算符、箭头、黑板体/花体…），其余走 `pylatexenc`，私有区垃圾码位丢弃 |
