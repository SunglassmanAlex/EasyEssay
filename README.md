...
| 左侧页码栏 | 每段标出在 PDF 中的起始页（`p.5`）；跨页段标区间（`p.5–6`）。**正文里不再插换页提示** |
| 伪代码 | 算法块按行渲染（等宽、保行号与缩进）。`algorithm` 环境上下也是横线，**长得像表格但不能当表格** |
| 真表格 | 重建二维网格后渲染成真正的 `<table>`（含跨列表头 `colspan`），左右两栏各完整一份，数值/行序/列义与原文一致；翻译按**整表**下发 |
...
│   ├── extract.py          PDF → 段落：双栏检测、页眉页脚过滤、跨页续段、公式碎片合并
│   ├── mathify.py          ★ 公式还原：数学字体识别、上下标重建、Unicode→LaTeX
│   ├── glyphnames.py       ★ 按字体字形名确定性还原大括号/求和号/大 ⊕（不靠模型猜）
│   ├── tables.py           ★ 表格/伪代码重建：booktabs 无竖线也能还原网格；区分表格·伪代码·图
...
- 表格里的 `\multirow`（跨行单元格）不支持：单元格在竖直方向居中，会并进相邻行。
  仍是一张可读的真表格，只是该格归属有偏差。完全靠空白对齐、**没有横线**的表格也识别不到。

### 怎么区分表格 / 伪代码 / 图

初版判据只看"两条等宽横线夹内容"，结果把三类东西都当成了表格（用户实测反馈）：

| 看到的现象 | 真相 | 判据 |
|---|---|---|
| `Algorithm 1: SEARCH(...)` 被拆成 5 列 | **伪代码**（`algorithm` 环境上下各一条 `\\hrule`） | Algorithm 标题 / 行号 / for·if·while·return / ← |
| 一段安全证明被切成 12 列 | **正文** | 挂的是 `Figure` 题注 → 丢弃，正文恢复为普通段落 |
| `efn:[1,128]`、`0.04 0.02` | **图的坐标轴刻度与图例** | 无 `Table` 题注 / `Figure` 题注 |

**最可靠的判据是题注**：论文里真表格一定配 `Table N:`，图配 `Figure N:`。
判定不确定的一律丢弃（不硬塞进表格），该区域正文就不再被抑制，会作为普通段落抽出来。

⚠️ 兜底路径（框线表格）也要过题注判据：**图的坐标轴 + 网格线本身就会围出闭合单元格**，
`find_tables(lines)` 会把整张折线图当表格。

### 表格怎么处理

论文里的表格绝大多数是 **booktabs 风格：只有顶/中/底三条横线，一根竖线都没有**。
PyMuPDF 的 `find_tables(strategy="lines")` 要求"闭合单元格"，对这种表**一个都找不到**；
`strategy="text"` 又太贪，会把整页正文吞进一个"表"里。两个策略都用不了，所以这里自己按几何重建：

| 步骤 | 判据 |
|---|---|
| 1. 定位表格 | 横线**等宽**（toprule / midrule / bottomrule）聚成一组；跨列用的 `\cmidrule` 宽度不同，自然落单 |
| 2. 定列槽 | **只取自表体行** —— 表头有跨列格，拿它定槽会把 5 列算成 8 列 |
| 3. 词→列 | 按"覆盖了哪些列槽"（**不能按词间空隙**：实测跨列间隙 2.8pt 比同格词距 3.5pt 还小） |
| 4. 跨列 | "组头吸收"：`Perplexity` 横跨列 1–2，下一行的 `(Lower is` / `better)` 位置上是两格、语义上属于它 |
| 5. 并行 | 行距 ≤ 0.8×字号算同一逻辑行的续行（窄列里表头换行） |

带框线的表格（每格一个矩形，横线被列切开）走 `find_tables(lines)` 兜底，两条路按 bbox 去重。

**伪代码有自己的处理**：按物理行取出（行号与缩进转成前导空格），翻译时按行下发
（行数与输入必须一致，否则拒绝），渲染成等宽代码块 —— 表格的"网格化"和普通段落的"合并成段"
都会毁掉伪代码的循环层级。

**翻译按整表下发**：给模型的是二维单元格数组而不是拍平的文本，模型返回同形状的数组，
再按原网格把跨列 `span` 套回去；行列数不符一律拒绝（宁可如实标未翻译，也不错位）。
