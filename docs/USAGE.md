...
- **看到 `⟦?⟧` 是什么意思**：PDF 里有些字形认不出来（字体表坏了）。这些字形几乎总是
  **大型括号、矩阵方括号、跨行大括号的上/下半截、分式线、求和号**。抽取阶段会把它标成 `⟦?⟧`
  而**不再静默丢弃**（静默丢弃正是"公式少括号、翻译跟着错"的根因），然后三层还原：
  ① 先读 PDF 字体的字形名**确定性地**还原（能查出来的绝不让模型猜）；
  ② 剩下的交给模型按上下文补齐（碎片段会带上前后邻居，并从相邻段合并回来）；
  ③ 校验有没有"伪装修复"（照抄占位符 / 换成 `?` / 换成空括号），不过关就严格重试。
  页面上 ⟦?⟧ 不会裸着显示成乱码，而是显示成一个带说明的小标记（鼠标悬停有解释）。
- 阅读页顶栏出现 **「修复公式段（N）」** 时点一下即可：只重译译文里仍有问题的段落
  （残留 `⟦?⟧`、`?` 或空括号），其它段落不动，花费远低于全文重译。
...
1. **题注最可靠** —— 论文里真表格一定配 `Table N:`，图配 `Figure N:`。
   靠它把"图的坐标轴刻度""被列槽切碎的正文"从表格候选里摘掉。
2. **伪代码看内容** —— `Algorithm 1:` 标题、多行以行号开头、
   出现 for/foreach/if/then/do/return/← 、同时有 `Input:` 与 `Output:`。
3. **没有题注时才看结构** —— 列数 2–8、行数 ≥2、格子填充率 ≥0.55。
4. **拿不准就丢弃** —— 不硬塞进表格：该区域的正文不再被抑制，会作为普通段落抽出来。

⚠️ 兜底路径（带框线的表格走 PyMuPDF）也要过题注判据：
**图的坐标轴 + 网格线本身就会围出"闭合单元格"**，否则整张折线图会被当成表格。

**三类东西的处理方式完全不同：**

| | 处理方式 |
|---|---|
| 表格 | 重建成二维网格 → 渲染真 `<table>`（含 `colspan`）；翻译整块下发（二维数组） |
| 伪代码 | 按**物理行**保留（行号 + 缩进）；翻译按行下发，**行数不一致直接拒绝**；渲染成等宽代码块 |
| 图/正文 | 不当作结构化块，交回普通段落（图只有刻度文字，正文照常逐段翻译） |

伪代码为什么不能走表格与普通段落：表格会把 `1 V ← ep // set of visited nodes`
拆成 5 个格子，普通段落会把连续几行并成一段 —— 两者都会毁掉"一行一条语句 + 缩进层级"。
